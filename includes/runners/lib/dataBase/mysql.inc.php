<?php
  ...
  /**
   * // <!-- phpDesigner :: Timestamp -->10/28/2012 12:06:57<!-- /Timestamp -->
  ...
  class MySQL
  {
	...
	$this->conf = array();
  			...
  			public function find_closest_metar($lat, $lon)
  			{
  						...
  						$res = mysql_query($sql, $dbh);
  						$x = '1';
  						while (list($icao, $name, $state, $country, $lon, $lat) = mysql_fetch_row($res))
  						{
  			...
  			public function get_global($city, $state, $country = "us")
  			{
                        ...
                        $state_zone='';
  						...
  						if (preg_match('/\d+/', $this->conf['zipcode']))
  						{
  			...
  			public function get_zip($zip)
  			{
  						...
  						$res = mysql_query($sql, $dbh);
  						list($place, $state, $lat, $lon) = mysql_fetch_row($res);
  						mysql_close($dbh);
  						...
  						$config = $this->get_global($place, $state);
  			...
  			function Marine($lat, $lon)
  			{
  						...
  						$sql = "SELECT  a.name, a.wxzone, a.tz, a.tzname, " . "(@distance:=(3963*ACOS(SIN(a.lat/57.3)*SIN($lat/57.3)+COS(a.lat/57.3)*COS($lat/57.3)*COS($lon/57.3 - a.lon/57.3)))) AS distance FROM marine_zones as a " .
  									" ORDER BY distance limit 0,1";
  						if ($this->debug)
  									print $sql;
  						$res = mysql_query($sql, $dbh);
  						...
  						$row = mysql_fetch_row($res);
  						mysql_close($dbh);
  						$this->smarty->assign('distance', round($row[4], 1));
  						if ($this->debug)
  									echo "\ndistance ?? " . $row[4] . "\n";
  						...
  						$this->conf['warnzone'] = $row[1];
  						$this->conf['name'] = $row[0];
  						$this->conf['tzname'] = (DST == '1') ? preg_replace("/S/", "D", $row[2]) : $row[2];
  						$this->conf['tzoff'] = $row[3] + DST;

  						$this->smarty->assign('location', $this->conf['name']);
  						$this->smarty->assign('state', '');
  						$this->smarty->assign('zone', $this->conf['warnzone']);
  			...
  			function get_icao($icao)
  			{
  						...
  						$res = mysql_query($sql, $dbh);
  						$x = '1';
  						list($icao, $name, $state, $country, $lon, $lat) = mysql_fetch_row($res);
  			...
  			function Get_multiple($loc)
  			{
  						...
  						if ($this->debug)
  									print "sql $sql\n";
  						while (list($name, $state, $lat, $lon) = mysql_fetch_row($res))
  						{
